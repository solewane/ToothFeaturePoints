...
bool BmpWriter::SaveImage(char* szPathName) {
	MY_BITMAPINFOHEADER BMIH;
	MY_BITMAPFILEHEADER bmfh;
	...
	BMIH.HeaderSize = sizeof(MY_BITMAPINFOHEADER);
	BMIH.Width = w;
	BMIH.Height = h;
	BMIH.Planes = 1;
	BMIH.BitCount = 24;
	...
	int nBitsOffset = sizeof(MY_BITMAPFILEHEADER) + BMIH.HeaderSize;
	...
	unsigned int nWrittenFileHeaderSize = fwrite(&bmfh, 1, sizeof(MY_BITMAPFILEHEADER), pFile);
	unsigned int nWrittenInfoHeaderSize = fwrite(&BMIH, 1, sizeof(MY_BITMAPINFOHEADER), pFile);
